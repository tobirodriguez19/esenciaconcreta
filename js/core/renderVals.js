  ...
  return { renderVals: function renderVals() {
    ...
    const adminProducts=prodList.map(p=>{
      ...
      return {...p, scentCount:vScents.length, isEditing, notEditing:!isEditing,
        ...
        allowCustomOrder:isEditing?d.allowCustomOrder:!!p.allowCustomOrder, onToggleAllowCustomOrder:()=>self.setDraftField('allowCustomOrder',!(isEditing?d.allowCustomOrder:p.allowCustomOrder)),
        customOrderEta:isEditing?d.customOrderEta:(p.customOrderEta||''), onCustomOrderEta:e=>self.setDraftField('customOrderEta',e.target.value),
        etaPresets:['24 h','48 h','3 días'].map(v=>({label:v,set:()=>self.setDraftField('customOrderEta',v)})),
    ...
    const coStatusDefs=[['pendiente','Pendiente'],['en_fabricacion','En fabricación'],['listo','Listo'],['entregado','Entregado'],['cancelado','Cancelado']];
    const customOrdersList=(s.customOrders||[]).map(o=>({
      id:o.id, productName:o.product_name, color:o.color_name||'', aroma:o.aroma_name||'', hasColor:!!o.color_name, hasAroma:!!o.aroma_name,
      qty:o.quantity, customerName:o.customer_name, customerPhone:o.customer_phone, comment:o.comment||'', hasComment:!!o.comment,
      status:o.status, statusOptions:coStatusDefs.map(([v,l])=>({value:v,label:l})), onStatus:e=>self.setCustomOrderStatus(o.id,e.target.value),
      notes:o.admin_notes||'', onNotes:e=>self.setCustomOrderNotes(o.id,e.target.value),
      dateFmt:new Date(o.created_at).toLocaleDateString('es-AR')
    }));
    ...
    const tabsDef=[['productos','Productos'],['categorias','Categorías'],['aromas','Aromas'],['ventas','Ventas'],['encargos','Pedidos por Encargo'],['destacados','Destacados'],['inicio','Inicio (El Oficio)']];
    ...
    return {
      ...
      customOrderOpen:s.customOrderOpen, customOrderDraft:s.customOrderDraft||{}, openCustomOrderModal:self.openCustomOrderModal, closeCustomOrderModal:self.closeCustomOrderModal,
      incCoQty:self.incCoQty, decCoQty:self.decCoQty, submitCustomOrder:self.submitCustomOrder,
      onCoNombre:e=>self.setCustomOrderField('nombre',e.target.value), onCoTelefono:e=>self.setCustomOrderField('telefono',e.target.value), onCoComentario:e=>self.setCustomOrderField('comentario',e.target.value),
      adminTabs, tabProductos:s.adminTab==='productos', tabCategorias:s.adminTab==='categorias', tabAromas:s.adminTab==='aromas', tabVentas:s.adminTab==='ventas', tabEncargos:s.adminTab==='encargos', tabDestacados:s.adminTab==='destacados', tabInicio:s.adminTab==='inicio',
      customOrdersList, customOrdersEmpty:customOrdersList.length===0,
      oficio,
      oficioImg,
